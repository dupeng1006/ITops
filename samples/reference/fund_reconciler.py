...
class FundAssetReconciler:
    ...
    def __init__(
        ...
        bulk_products: Optional[set] = None,
        ...
        Args:
            ...
            bulk_products: 大宗产品代码集合
            diff_threshold: 差异阈值（%）
            similarity_threshold: 模糊匹配相似度阈值
            logger: 日志记录器
        """
        self.rename_map = rename_map or DEFAULT_RENAME_MAP.copy()
        self.bulk_products = bulk_products or DEFAULT_BULK_PRODUCTS.copy()
    ...
    def mark_bulk_products(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        标记大宗产品
        """
        df['是否大宗'] = df['信托计划代码'].isin(self.bulk_products)
        return df
...
使用示例:
  ...
  python fund_reconciler.py fund.xls netvalue.xls --threshold 0.5 --similarity 0.6
    ...
    parser.add_argument('-c', '--config', help='JSON配置文件路径（包含rename_map和bulk_products）')
    ...
    bulk_products = DEFAULT_BULK_PRODUCTS.copy()
    
    if args.config:
        with open(args.config, 'r', encoding='utf-8') as f:
            config = json.load(f)
            rename_map.update(config.get('rename_map', {}))
            bulk_products.update(config.get('bulk_products', []))
