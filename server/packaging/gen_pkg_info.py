...
CHANGELOG = """\
v0.7.3（2026-07-24）深市接口规范字段说明库入库（沪深双库齐备）
  - 内置字段说明库扩充：预解析官方《深市登记结算数据接口规范（结算参与人版
    Ver5.17）》PDF 生成，26 个接口 / 552 条字段说明；与沪市库（V3.102，
    42 接口 / 655 条）并列生效，合计 68 接口 / 1207 条
  - 深市接口按文件名前缀自动匹配：sjsmx→清算明细库、sjsjg→明细结果库、
    sjsye→资金余额信息库、sjsqs→资金清算汇总库等，最长前缀优先
    （sjsqs0 优先于 sjsqs）；szyctz 字段定义沿用 sjsmx（官方文档注明）
  - 抽取结果经人工逐接口抽查（表头折行/描述折行/跨页续表/备注防污染
    均已核对），深市文件表头同样显示官方中文说明
  - 沪市文件匹配不受影响（ZQYEJS329.713 等仍命中沪市库）
v0.7.2（2026-07-24）DBF 查看自动匹配中登官方接口字段说明
  - DBF 数据查看升级：按文件名前缀自动匹配中登业务接口（zqye→证券余额
    对账、zqbd→证券变动、jsmx01→结算明细第一批次、ywhb→业务回报等），
    表头字段名下方显示官方中文说明，悬停可见完整填写说明
  - 内置字段说明库：预解析官方《登记结算数据接口规范（结算参与人版
    V3.102）》PDF 生成，42 个接口 / 655 条字段说明，随包发布零依赖；
    页面顶部展示接口匹配信息（接口名/规范来源/命名约定/说明覆盖率）
  - 匹配不到接口的文件正常展示（仅无字段说明），不影响使用
  - 深市规范（Ver5.17）库按用户需求后续扩充
v0.7.1（2026-07-24）DBF 查看兼容中登日期后缀文件
  - DBF 文件识别改为按文件内容嗅探（dBase/FoxPro 版本魔数 + 头部结构自洽
    校验），不再限制 .dbf 扩展名——中登 ZQYEJS329.713 等"内容是 DBF、
    扩展名按日期命名"的文件可直接上传解析（已用真实中登文件验证：
    Visual FoxPro 0x30 / GBK / 13 字段 398 行解析正确）
  - 非 DBF 内容文件给出明确中文提示（含首字节信息，便于排查）
v0.7.0（2026-07-24）新增 DBF 数据查看
  - 新增 DBF 数据查看（admin/operator）：上传 dBase/FoxPro .dbf 文件，
    解析为表格分页展示（字段名/类型/长度 + 前 2000 行预览），支持一键
    导出全量 Excel（限 20 万行）；纯只读，绝不修改原文件
  - 编码自动识别：GBK → UTF-8 → 单字节兜底（兜底层会明确提示中文可能乱码）
  - 操作留痕审计：查看与导出均记录文件名、字段数、行数、编码
  - 依赖新增 dbfread 2.0.7（纯 Python，已冻结进部署包，目标机零安装）
v0.6.9（2026-07-23）页面上传包在线升级（升级零手工脚本）
  - 【重要】新增 系统管理-版本升级：admin 在页面直接上传新版部署包 zip，
    服务端自动完成 校验包结构 → 备份数据到 backups → 替换程序 → 自动重启，
    全程无需执行任何 bat 脚本；数据（账号/规则/特殊产品/模板/数据源/
    Trello/历史任务/归档）在固定数据目录原地不动，架构上与程序彻底分离
  - 升级动作全程审计留痕：发起升级（谁、从哪版到哪版）+ 新进程启动时
    自动核对版本并记录 升级完成/异常（系统日志查询可见）
  - 新增 GET /api/admin/system/version：页面展示当前版本、安装/数据目录
  - 安全：仅 admin 可用；仅冻结部署版开放；zip 结构强校验（必须含
    app/o32-server.exe）；上传限 500MB；升级中防并发重复触发；
    数据目录绝不解包覆盖，仅字典库 dictionary.db（纯参考数据）随包更新
v0.6.8（2026-07-23）升级机制改为数据自动保留 + 品牌 favicon
  - 【重要】升级机制重构：改为“固定安装目录 + 覆盖式升级”——数据 data 目录
    固定在安装目录原地不动，升级只覆盖 app 程序文件；账号/规则/模板/数据源/
    Trello/历史任务/归档全部自动保留，无需再手动执行数据迁移
  - upgrade.bat 重写为一键覆盖升级：自动定位安装目录（默认 D:\O32-Ops，可用
    install-dir.txt 或参数指定）、自动停服务、自动备份数据到 backups、覆盖 app、
    保留数据，双击即完成，无需选目录
  - 浏览器标签 favicon：由默认灰色地球改为 Allianz 圆形徽标（官方 logo 提取，
    蓝 #003781 透明底），与公司标准产品一致
v0.6.7（2026-07-23）品牌 Logo 替换为官方 Allianz 标识
  - 顶栏与登录页 Logo 由内联 SVG 近似复刻升级为官方 Allianz 标识图片
    （Allianz 字标 + 圆环三立柱徽标），透明底处理
  - 深蓝顶栏用白色版、登录页浅色背景用蓝色版
  - 按用户确认：不加动态水印
v0.6.6（2026-07-23）界面视觉对齐公司标准产品风格
  - 布局骨架重构：由“左侧深蓝菜单+顶部白标题栏”改为“顶部安联蓝渐变导航栏
    （#003781→#005EB8）+ 左侧白色菜单”，对齐公司标准外采产品观感
  - 顶栏：Allianz 品牌 Logo（内联 SVG 复刻圆环三立柱徽标 + 字标）+ 系统名 + 右侧用户姓名下拉
  - 左侧菜单白色化：普通项深灰文字、当前项安联蓝文字 + 浅蓝底 + 左侧蓝色竖条；
    全部功能菜单/权限/路由保持不变
  - Element Plus 主色统一为安联蓝 #005EB8（brand.css CSS 变量覆盖）
  - 登录页：品牌蓝渐变背景 + 品牌 Logo + 主色对齐
  - 纯视觉重构，零业务逻辑改动
v0.6.5（2026-07-23）Trello 看板体验优化 + 修改人姓名化 + API Key 加密
  - Trello 工作看板：看板间距收紧、横向滚动条贴内容底部；看板超过 4 个
    时出现“快速定位”下拉，选中平滑滚动跳转
  - Trello 卡片描述：Spoiler 短链等 [文本](URL) 与裸链接渲染为可点击超链接
    （域名缩写显示），不再展示一大串原始长链接；自动转义防 XSS
  - 配置中心各模块“修改人”列：由用户编号升级显示为人员姓名（系统初始化
    生成的数据行显示“系统初始化”），覆盖规则配置/数据源/查询模板/科目规则/
    系统参数/Trello 配置
  - Trello API Key 改为 Fernet 加密存储（与 Token 一致），接口返回掩码
    ********；数据库迁移 v7 自动把存量明文 Key 加密，无需手工处理
  - 数据库迁移 v5 前置守卫：启动时校验 sys_audit_log 等表存在性，避免旧库缺表崩溃
  - 新增测试门禁：审计服务 52 项 + Trello 服务 50 项 API 测试；迁移单测补 v7 加密 6 项
...
v0.6.3（2026-07-22）系统日志查询 + 审计 MAC/操作菜单记录
  - 系统管理下新增「系统日志查询」页：操作人员（编号/姓名/部门）、
    ...
    类型/时间区间/IP-MAC 组合过滤与分页（仅 admin 可见）
  - 审计留痕升级：sys_audit_log 增加 mac/menu 两列（幂等迁移 v5，
    存量日志无损、老数据显示"—"）；MAC 由服务端 ARP 解析（同网段
    自动获取、零权限零依赖，跨网段显示"—"）；全部 50 个审计点补
    操作菜单（登录页/数据核对中心/规则配置/数据源/系统管理等）
  - 新增接口：GET /api/audit-logs（分页过滤）、/api/audit-logs/menus
v0.6.2（2026-07-21）数据字典收藏置顶优化
  - 收藏表在搜索结果与浏览列表中均置顶显示（后端 CASE 排序 +
    前端 sortedTableItems 双保险），收藏项金色实心星 + 橙色高亮底色
  - 移除左侧独立的"常用收藏"置顶区块，收藏表融入列表不再重复展示
  - 界面更简洁；README 同步补齐到 v0.6.1 现状（更名/接口/测试门禁）
